...
if (-e  tmp/OR/$group/d4.de_uno.txt) then
  ...
  foreach target ($Etargets)
    cat  tmp/METADATA/$target.[fr].introns ZZZZZ tmp/OR/$group/d4.de_uno.txt | gawk -F '\t' '/^ZZZZZ/{zz++;next;}{if(zz<1){a1=$2+0;a2=$3+0;av[$1,a1+0,a2+0] =1 ; nAv++; next; }}{if($2+0==0)next;min1=minS;if($5 == "gt_ag" || $5 == "gt_ag")min1=minS/200;if($4<min1)next;nn++;if(av[$1,0+$2,0+$3]==1){ if($5 == "gt_ag" || $5 == "gt_ag") { inGtAg++;}inAv++;n4in+=$4;}else { if($5 == "gt_ag" || $5 == "gt_ag") { outGtAg++;} outAv++;n4Out+=$4;}}END{if(nn==0)nn=1;if(nAv==0)nAv=1;sp=inAv/nn;sn=inAv/nAv;printf("Ali %s\n-D Candidate_introns uno\n-D Candidate_introns %s\n",group,target);printf("Candidate_introns %s %d:%d:%d ",target, nn,inGtAg+outGtAg,nn-inGtAg-outGtAg); printf(" In_%s %d:%d:%d ",target, inAv, inGtAg,inAv-inGtAg); printf ("Not_in_%s %d:%d:%d ", target, outAv, outGtAg, outAv - outGtAg); printf (" Specificity %.2f Sensitivity %.2f Known_Support %d New_support %d New_minS %d\n\n", 100*sp, 100*sn, n4in, n4Out, minS) ;}' target=$target group=$group minS=$minS >> tmp/OR/$group/d4.candidate_introns.ace
  end
